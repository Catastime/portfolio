...
function DitheredWaves({
  ...
  useEffect(() => {
    ...
    if (res.x !== w || res.y !== h) {
      res.set(w, h);
      const matU = mesh.current?.material?.uniforms;
      if (matU) matU.resolution.value.set(w, h);
    }
  }, [size, gl]);
  ...
  useFrame(({ clock }) => {
    ...
    if (!disableAnimation) {
      ...
      if (matU) matU.time.value = u.time.value;
    }
    ...
    if (matU) {
      if (matU.waveSpeed.value !== waveSpeed) matU.waveSpeed.value = waveSpeed;
      if (matU.waveFrequency.value !== waveFrequency) matU.waveFrequency.value = waveFrequency;
      if (matU.waveAmplitude.value !== waveAmplitude) matU.waveAmplitude.value = waveAmplitude;
    }

    if (!prevColor.current.every((v, i) => v === waveColor[i])) {
      u.waveColor.value.set(...waveColor);
      if (matU) matU.waveColor.value.set(...waveColor);
      prevColor.current = [...waveColor];
    }

    if (!prevBackgroundColor.current.every((v, i) => v === backgroundColor[i])) {
      u.backgroundColor.value.set(...backgroundColor);
      if (matU) matU.backgroundColor.value.set(...backgroundColor);
    ...
    if (matU) {
      matU.enableMouseInteraction.value = enableMouseInteraction ? 1 : 0;
      matU.mouseRadius.value = mouseRadius;
    }

    if (enableMouseInteraction) {
      u.mousePos.value.copy(mouseRef.current);
      if (matU) matU.mousePos.value.copy(mouseRef.current);
    }
  });
